...
- Record page: added a "Frames per recording" slider (50 to 500, step 50, default 100, remembered in the browser). Reasoning: frames within one take are near-copies at 10 per second, so longer takes only help if angle/distance/position keep changing; sessions and people matter more than take length. The recorder's give-up cap now scales with the target (2x the expected time). Contribute page stays at a fixed 100. Tested with a fake clock at targets 50, 300 and 500.
